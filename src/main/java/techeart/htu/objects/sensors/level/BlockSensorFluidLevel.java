...
public class BlockSensorFluidLevel extends HTUBlock
{
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit)
    {
        if(!worldIn.isRemote() && player.getHeldItem(handIn).isEmpty())
        {
            ...
            ModUtils.playerInfoMessage(LocalizedMessages.getInfoFluidLevel(measured), player);
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos)
    {
        if(facing == Direction.DOWN)
        {
            ...
            TileEntitySensorFluidLevel te = (TileEntitySensorFluidLevel) worldIn.getTileEntity(currentPos);
            if(te == null)
                return Blocks.AIR.getDefaultState();
            ((TileEntitySensorFluidLevel)te).updateTarget();
//            MainClass.LOGGER.error("Invalid TileEntity at pos {} {} {}", currentPos.getX(), currentPos.getY(), currentPos.getZ());
        }
